...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, early_stopping, output_dir, scaler):
    ...
    csv_path = os.path.join(output_dir, 'epoch_summary_resnet50.csv')
...
def plot_metrics(train_losses, val_losses, train_accs, val_accs,
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'loss_plot_resnet50.png'))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'accuracy_plot_resnet50.png'))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'prf1_plot_resnet50.png'))
    plt.close()
    ...
    plt.ylabel('True')
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_resnet50.png'))
    plt.close()
...
def setup_logging(output_dir):
    log_file = os.path.join(output_dir, 'training_resnet50.log')
...
def main(args):
    ...
    logger.info("Starting training for ResNet-50")

    try:
        model = models.resnet50(pretrained=True)
        model.fc = nn.Linear(model.fc.in_features, num_classes)
        log_model_stats(model, "ResNet-50", logger)
    except Exception as e:
    ...
    scaler = GradScaler()
    early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(output_dir, 'resnet50_best.pth'))

    try:
        model = train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, early_stopping, output_dir, scaler)
    except Exception as e:
        logger.error(f"Error during training ResNet-50: {e}")
        traceback.print_exc()
        return
    ...
    try:
        ...
        logger.error(f"Error saving final model for ResNet-50: {e}")
        traceback.print_exc()
        return
    ...
    logger.info("Completed training for ResNet-50")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ResNet-50 for tomato leaf disease classification")
