...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, early_stopping, output_dir, scaler):
    model.to(device)
    ...
    csv_path = os.path.join(output_dir, 'epoch_summary_mobilenetv2.csv')
    ...
    plot_metrics(train_losses, val_losses, train_accs, val_accs,
                 train_precisions, train_recalls, train_f1s,
                 val_precisions, val_recalls, val_f1s,
                 val_labels, val_preds, output_dir)
...
def plot_metrics(train_losses, val_losses, train_accs, val_accs,
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'loss_plot_mobilenetv2.png'))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'accuracy_plot_mobilenetv2.png'))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'prf1_plot_mobilenetv2.png'))
    plt.close()
    ...
    plt.ylabel('True')
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_mobilenetv2.png'))
    plt.close()
...
def setup_logging(output_dir):
    log_file = os.path.join(output_dir, 'training_mobilenetv2.log')
...
def main(args):
    ...
    try:
        train_dataset = SafeImageFolder(train_dir)
        val_dataset = SafeImageFolder(val_dir)
    except Exception as e:
    ...
    logger.info("Starting training for MobileNetV2")

    try:
        model = models.mobilenet_v2(pretrained=True)
        model.classifier[1] = nn.Linear(model.classifier[1].in_features, num_classes)
        log_model_stats(model, "MobileNetV2", logger)
    except Exception as e:
    ...
    scaler = GradScaler()
    early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(output_dir, 'mobilenetv2_best.pth'))

    try:
        model = train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, early_stopping, output_dir, scaler)
    except Exception as e:
        logger.error(f"Error during training MobileNetV2: {e}")
        traceback.print_exc()
        return

    try:
        torch.save(model.state_dict(), os.path.join(output_dir, 'mobilenetv2_final.pth'))
    except Exception as e:
        logger.error(f"Error saving final model for MobileNetV2: {e}")
        traceback.print_exc()
        return
    ...
    logger.info("Completed training for MobileNetV2")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train MobileNetV2 for tomato leaf disease classification")
    ...
    parser.add_argument('--data_dir', type=str,
                        default="C:/Users/nisha/OneDrive/Desktop/ViT-RoT/dataset/tomato_leaf_dataset",
                        help="Path to the dataset root directory")
    parser.add_argument('--test_image', type=str,
                        default="C:/Users/nisha/OneDrive/Desktop/ViT-RoT/dataset/tomato_leaf_dataset/val",
