...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, early_stopping, variant, output_dir):
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            optimizer.zero_grad()
            outputs = model(images).logits  # Access logits from transformers models
...
def main(args):
    ...
    # List of PVT v1 variants to train
    variants = [
        ('tiny', 'Zetatech/pvt-tiny-224'),
        ('small', 'Zetatech/pvt-small-224'),
        ('base', 'Zetatech/pvt-medium-224'),  # medium is mapped to base
        ('large', 'Zetatech/pvt-large-224')
    ]
    ...
    for variant, model_name in variants:
        ...
        early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(output_dir, f'pvt_{variant}_best.pth'))
        ...
        # Save final models
        try:
            torch.save(model.state_dict(), os.path.join(output_dir, f"pvt_{variant}_final.pth"))
        except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Zetatech PVT v1 variants for tomato leaf disease classification")
